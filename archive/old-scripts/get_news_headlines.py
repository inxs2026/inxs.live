...
def main():
    ...
    if cbc:
        ...
        print("   • CBC: Unable to fetch")
    
    print()
    print("🇺🇸 USA NEWS - TOP 3\n")
    
    # Reuters
    reuters = get_rss_headlines('https://feeds.reuters.com/reuters/topNews', 3)
    if reuters:
        for i, headline in enumerate(reuters, 1):
            print(f"   {i}. {headline}")
    else:
        # Try AP News as backup
        ap = get_rss_headlines('https://feeds.apnews.com/rss/topnews', 3)
        if ap:
            for i, headline in enumerate(ap, 1):
                print(f"   {i}. {headline}")
        else:
