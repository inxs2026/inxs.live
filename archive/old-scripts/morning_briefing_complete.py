...
- News headlines WITH LINKS (RSS feeds - free)
- Stock market headlines + TSX direction WITH LINKS
"""
import subprocess
import datetime
import json
...
def get_rss_headlines_with_links(url, count=3):
    """Fetch RSS feed headlines WITH links"""
    try:
        result = subprocess.run(
...
def get_market_data():
    """Get stock market headlines and TSX direction WITH LINKS"""
    headlines = []
    ...
    for feed in market_feeds:
        headlines = get_rss_headlines_with_links(feed, 3)
        if headlines:
            break
...
def main():
    ...
    print()
    cbc = get_rss_headlines_with_links('https://www.cbc.ca/webfeed/rss/rss-topstories', 3)
    if cbc:
        for i, (title, link) in enumerate(cbc, 1):
            if link:
                print(f"   {i}. {title}")
                print(f"      {link}")
            else:
    ...
    for feed in us_feeds:
        us_headlines = get_rss_headlines_with_links(feed, 3)
        if us_headlines:
            break
    
    if us_headlines:
        for i, (title, link) in enumerate(us_headlines, 1):
            if link:
                print(f"   {i}. {title}")
                print(f"      {link}")
            else:
    ...
    if market_headlines:
        ...
        for i, (title, link) in enumerate(market_headlines, 1):
            if link:
                print(f"   {i}. {title}")
                print(f"      {link}")
            else:
